...
static void pick_color(win_content_t *wc, picker_t *picker, sfVector2f pos)
{
    ...
    wc->draw->color = sfImage_getPixel
        (picker->image, image_pos.x, image_pos.y);
}
...
void init_color_picker(toolbar_t *toolbar)
{
    ...
    sfRectangleShape_setSize(picker->rect,
        (sfVector2f){COLOR_PICKER_WIDTH, COLOR_PICKER_HEIGHT});
    sfRectangleShape_setPosition(picker->rect,
        (sfVector2f){COLOR_PICKER_X, TOP_BAR_HEIGHT +
